...
from inference_mr import eval_epoch_neg, start_inference
...
def train_epoch(model, criterion, train_pos_loader, train_neg_loader, train_indomain_neg_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_pos_loader),
        ...
        neg_indomain_batch = next(iter(train_indomain_neg_loader))
        ...
        model_indomain_neg_inputs, targets_indomain_neg = prepare_batch_inputs_mr(neg_indomain_batch[1], opt.device, non_blocking=opt.pin_memory)
        ...
        loss_outofdomain_neg_dict = criterion(outputs_neg, targets_neg)
        loss_outofdomain_neg_dict["loss_b"] = loss_outofdomain_neg_dict["loss_b"] * 0
        loss_outofdomain_neg_dict["loss_g"] = loss_outofdomain_neg_dict["loss_g"] * 0
        weight_dict = criterion.weight_dict
        losses_outofdomain_neg = sum(loss_outofdomain_neg_dict[k] * weight_dict[k] for k in loss_outofdomain_neg_dict.keys() if k in weight_dict)

        outputs_indomain_neg = model(**model_indomain_neg_inputs)
        loss_indomain_neg_dict = criterion(outputs_indomain_neg, targets_indomain_neg)
        loss_indomain_neg_dict["loss_b"] = loss_indomain_neg_dict["loss_b"] * 0
        loss_indomain_neg_dict["loss_g"] = loss_indomain_neg_dict["loss_g"] * 0
        weight_dict = criterion.weight_dict
        losses_indomain_neg = sum(loss_indomain_neg_dict[k] * weight_dict[k] for k in loss_indomain_neg_dict.keys() if k in weight_dict)
        time_meters["model_forward_time"].update(time.time() - timer_start)

        losses = losses_pos + opt.out_coef * losses_outofdomain_neg + opt.in_coef * losses_indomain_neg
        ...
        loss_outofdomain_neg_dict["ood_neg_loss_overall"] = float(losses_outofdomain_neg)  # for logging only
        loss_indomain_neg_dict["id_neg_loss_overall"] = float(losses_indomain_neg)
        loss_pos_dict["total_loss"] = float(losses) # for logging only
        for k, v in loss_pos_dict.items():
            loss_meters[k].update(float(v) * weight_dict[k] if k in weight_dict else float(v))

        for k, v in loss_outofdomain_neg_dict.items():
            loss_meters['outofdomain_neg_' + k].update(float(v) * weight_dict[k] if k in weight_dict else float(v))

        for k, v in loss_indomain_neg_dict.items():
            loss_meters['indomain_neg' + k].update(float(v) * weight_dict[k] if k in weight_dict else float(v))
...
def train(model, criterion, optimizer, lr_scheduler, train_pos_dataset, train_neg_dataset, train_indomain_neg_dataset, val_dataset, val_neg_dataset, opt):
    ...
    train_indomain_neg_loader = DataLoader(
        train_indomain_neg_dataset,
        collate_fn=start_end_collate_mr,
        # batch_size=opt.bsz,
        batch_size=32,
        num_workers=opt.num_workers,
    ...
    if opt.start_epoch is None:
        ...
        # start_epoch = 0 if opt.eval_init else 0
    else:
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        if epoch_i > -1:
            train_epoch(model, criterion, train_pos_loader, train_neg_loader, train_indomain_neg_loader, optimizer, opt, epoch_i, tb_writer)
            lr_scheduler.step()
        eval_epoch_interval = opt.eval_epoch
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            with torch.no_grad():
                metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths,\
                metrics_no_nms_neg, metrics_nms_neg, eval_loss_meters_neg, latest_file_paths_neg = \
                    eval_epoch_neg(model, val_dataset, val_neg_dataset, opt, save_submission_filename, epoch_i, criterion, tb_writer)
...
def start_training():
    ...
    train_dataset_config = dict(
        ...
        neg_indomain_train_path=opt.neg_indomain_train_path,
    ...
    train_indomain_neg_dataset_config = dict(
        dset_name=opt.dset_name,
        pos_train_path=opt.pos_train_path,
        neg_train_path=opt.neg_indomain_train_path,
        neg_indomain_train_path=opt.neg_indomain_train_path,
        v_feat_dirs=opt.v_feat_dirs,
        q_feat_dir=opt.t_feat_dir,
        v_feat_dim=opt.v_feat_dim,
    ...
    train_dataset_config["neg_indomain_train_path"] = opt.neg_indomain_train_path
    train_pos_dataset = DatasetMRPos(**train_dataset_config)
    train_neg_dataset = DatasetMRNeg(**train_dataset_config)
    train_neg_indomain_dataset = DatasetMRNeg(**train_indomain_neg_dataset_config)
    ...
    train(model, criterion, optimizer, lr_scheduler, train_pos_dataset, train_neg_dataset, train_neg_indomain_dataset, eval_dataset, eval_neg_dataset, opt)
