...
class DatasetVLP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if meta['qid'] is not None:
            ...
            if relevant_windows.size() == torch.Size([1, 0]):
                diff_left = torch.tensor(0)
                diff_right = torch.tensor(0)
            else:
...
class DatasetMR(Dataset):
    ...
    def __getitem__(self, index):
        ...
        model_inputs["span_labels_nn"] = nn_window_ts
        model_inputs["timestamp_window"] = 1 * (model_inputs["timestamp"][:,0] >= nn_window_ts[:,0])  & (model_inputs["timestamp"][:,1] <= nn_window_ts[:,1])
        if torch.sum(nn_window_ts) == 0:   # added by Kevin to ensure negatives have ground truth labels of all 0
            # print('YES')
            model_inputs["timestamp_window"] = model_inputs["timestamp_window"] * 0
        # print(model_inputs["timestamp_window"])
        # for activitynet.
        # if model_inputs["timestamp_window"].sum() < 1: # commented out by Kevin because this affects the negative labels
            # idx = int(meta['relevant_windows'][0][0] / self.clip_len)
            # idx = max(0, min(idx, ctx_l-1))
            # model_inputs["timestamp_window"][idx] = 1
        ...
        if self.load_labels:
            ...
            if 'saliency_scores' in meta.keys():
                ...
                limit = meta["relevant_clip_ids"].index(ctx_l) if (np.array(meta["relevant_clip_ids"]) >= ctx_l).any() else None
                model_inputs["saliency_scores"][meta["relevant_clip_ids"][:limit]] = torch.tensor(np.mean(np.array(meta["saliency_scores"][:limit]), -1))
                if meta["saliency_scores"] == [[]]:
                    model_inputs["saliency_pos_labels"] = []
                    model_inputs["saliency_neg_labels"] = self.get_saliency_labels_neg_only(meta["relevant_clip_ids"], meta["saliency_scores"], ctx_l)
                else:
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"] = \
                        self.get_saliency_labels(meta["relevant_clip_ids"], meta["saliency_scores"], ctx_l)
                # print('saliency pos and neg labels', model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"])
    ...
    def get_saliency_labels_neg_only(self, rel_clip_ids, scores, ctx_l, max_n=1):
        # Added by Kevin
        """Sum the scores from the three annotations, then take the two clips with the
        maximum scores as positive, and two with the minimum scores as negative.
        Args:
        ...
        potential_indices = range(ctx_l)
        neg_clip_indices = random.sample(potential_indices, 1)

        return neg_clip_indices
    ...
    def _get_query_feat_by_qid(self, qid):
        if self.use_cache > 0:
            try:
                ...
                q_feat = np.zeros((10, self.q_feat_dim)).astype(np.float32)
                print('error loading text feature')
            return  torch.from_numpy(q_feat)
...
class DatasetMRPos(Dataset):
    ...
    def __init__(self, dset_name, pos_train_path, neg_train_path, neg_indomain_train_path, v_feat_dirs, q_feat_dir, v_feat_dim, q_feat_dim,
                 q_feat_type="last_hidden_state",
                 max_q_l=32, max_v_l=75, data_ratio=1.0, ctx_mode="video",
                 normalize_v=True, normalize_t=True, load_labels=True,
                 clip_len=2, max_windows=5, span_loss_type="l1", txt_drop_ratio=0,
                 use_cache=-1, fix_len=-1, add_easy_negative=1, easy_negative_only=-1):
        self.dset_name = dset_name
        self.pos_train_path = pos_train_path[0] if isinstance(pos_train_path, list) else pos_train_path
        self.neg_train_path = neg_train_path[0] if isinstance(neg_train_path, list) else neg_train_path
        self.data_ratio = data_ratio
        self.v_feat_dirs = v_feat_dirs \
            if isinstance(v_feat_dirs, list) else [v_feat_dirs]
        self.q_feat_dir = q_feat_dir
        ...
        if "val" in pos_train_path or "test" in pos_train_path:
            assert txt_drop_ratio == 0
        ...
        # data
        self.pos_data, self.neg_data = self.load_data()
        ...
        if self.use_cache > 0:
            print('Loading the off-line features...')
            dset_dir = os.path.join('data', self.dset_name)
            vid_keys = [meta['vid'] for meta in self.pos_data] + [meta['vid'] for meta in self.neg_data]
            qid_keys = [meta['qid'] for meta in self.pos_data] + [meta['qid'] for meta in self.neg_data]
    ...
    def load_data(self):
        pos_datalist = load_jsonl(self.pos_train_path)
        neg_datalist = load_jsonl(self.neg_train_path)
        if self.data_ratio != 1:
            n_examples = int(len(pos_datalist) * self.data_ratio)
            pos_datalist = pos_datalist[:n_examples]
            logger.info("Using {}% of the data: {} examples"
                        .format(self.data_ratio * 100, n_examples))
    ...
    def __getitem__(self, index):
        ...
        if 'test' in self.pos_train_path and 'qvhighlights' in self.dset_name:
            meta["relevant_windows"] = [[0, 150]]
        relevant_windows = torch.Tensor(meta["relevant_windows"])
        ...
        model_inputs["span_labels_nn"] = nn_window_ts
        model_inputs["timestamp_window"] = 1 * (model_inputs["timestamp"][:, 0] >= nn_window_ts[:, 0]) & (
                    model_inputs["timestamp"][:, 1] <= nn_window_ts[:, 1])
        ...
        if self.load_labels:
            ...
            if 'saliency_scores' in meta.keys():
                model_inputs["saliency_scores"] = torch.zeros(ctx_l).double()
                limit = meta["relevant_clip_ids"].index(ctx_l) if (
                            np.array(meta["relevant_clip_ids"]) >= ctx_l).any() else None
                model_inputs["saliency_scores"][meta["relevant_clip_ids"][:limit]] = torch.tensor(
                    np.mean(np.array(meta["saliency_scores"][:limit]), -1))
                if meta["saliency_scores"] == [[]]: # Added by Kevin
                    model_inputs["saliency_pos_labels"] = []
                    model_inputs["saliency_neg_labels"] = self.get_saliency_labels_neg_only(meta["relevant_clip_ids"],
                ...
                model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"] = \
                    self.get_saliency_labels_sub_as_query(meta["relevant_windows"][0], ctx_l)  # only one gt
                model_inputs["saliency_pos_labels"] = [
                    random.choice(torch.where(model_inputs['saliency_scores'])[0].tolist())]
    ...
    def _get_query_feat_by_qid(self, qid):
        if self.use_cache > 0:
            try:
                ...
                print('NO TEXT FEATURES')
                q_feat = np.zeros((10, self.q_feat_dim)).astype(np.float32)
                logger.info(f"{q_feat_path} is not in cache")
            return torch.from_numpy(q_feat)
...
class DatasetMRNeg(Dataset):
    ...
    def __init__(self, dset_name, pos_train_path, neg_train_path, neg_indomain_train_path, v_feat_dirs, q_feat_dir, v_feat_dim, q_feat_dim,
        ...
        self.pos_train_path = pos_train_path[0] if isinstance(pos_train_path, list) else pos_train_path
        self.neg_train_path = neg_train_path[0] if isinstance(neg_train_path, list) else neg_train_path
        self.neg_indomain_train_path = neg_indomain_train_path[0] if isinstance(neg_train_path, list) else neg_indomain_train_path
        self.data_ratio = data_ratio
        self.v_feat_dirs = v_feat_dirs \
            if isinstance(v_feat_dirs, list) else [v_feat_dirs]
        self.q_feat_dir = q_feat_dir
        ...
        # data
        self.neg_data = self.load_data()

        self.v_feat_types = [feat_dir.split('/')[-1] for feat_dir in self.v_feat_dirs]
        t_feat_type = q_feat_dir.split('/')[-1]

        # Load the list of videos for training so that they can be randomly selected for the negative queries
        train_jsonl = load_jsonl(f'data/{self.dset_name}/metadata/{self.dset_name}_train.jsonl')
        self.train_vid_ids = []
        for line in train_jsonl:
            self.train_vid_ids.append(line['vid'])
        self.train_vid_ids = list(set(self.train_vid_ids))

        if self.use_cache > 0:
            print('Loading the off-line features...')
            dset_dir = os.path.join('data', self.dset_name)
            vid_keys = [meta['vid'] for meta in self.neg_data]
            vid_keys_full = self.train_vid_ids
            qid_keys = [meta['qid'] for meta in self.neg_data]

            self.vid_cache = {}
            for v_feat_type in self.v_feat_types:
                assert 'vid' in v_feat_type
                with h5py.File(os.path.join(dset_dir, 'h5py', v_feat_type + '.hdf5'), 'r') as f:
                    self.vid_cache[v_feat_type] = {key: f[str(key)][:] for key in tqdm(vid_keys_full)}
    ...
    def load_data(self):
        ...
        neg_datalist = load_jsonl(self.neg_train_path)
        if self.data_ratio != 1:
            n_examples = int(len(neg_datalist) * self.data_ratio)
            pos_datalist = neg_datalist[:n_examples]
            logger.info("Using {}% of the data: {} examples"
                        .format(self.data_ratio * 100, n_examples))
        return neg_datalist
    ...
    def __getitem__(self, index):
        meta = self.neg_data[index]
        # print('META', meta)

        model_inputs = dict()
        model_inputs["query_feat"] = self._get_query_feat_by_qid(meta["qid"])  # (Dq, ) or (Lq, Dq)

        # OPTION FOR RANDOM SELECTION OF VIDEO FOR NEGATIVE QUERIES
        random_neg_video = True


        if self.use_video:
            if random_neg_video:
                # for random videos you need to get the list of videos and select one at random here - Kevin
                video = random.choice(self.train_vid_ids)
                model_inputs["video_feat"] = self._get_video_feat_by_vid(video)
            else:
                model_inputs["video_feat"] = self._get_video_feat_by_vid(meta["vid"])  # (Lv, Dv)
            ctx_l = len(model_inputs["video_feat"])
        else:
...
def start_end_collate_mr(batch):
    ...
    batched_data = dict()
    # print('first', batch[0]["model_inputs"]["saliency_pos_labels"])
    # print('second', batch[1]["model_inputs"]["saliency_pos_labels"])
    # print('third', batch[2]["model_inputs"]["saliency_pos_labels"])
    for k in model_inputs_keys:
        if k == "span_labels":
            batched_data[k] = [dict(spans=e["model_inputs"]["span_labels"]) for e in batch]
            continue
        if k in ["saliency_pos_labels", "saliency_neg_labels"]:
            # print(torch.LongTensor([e['model_inputs'][k] for e in batch]))
...
def prepare_batch_inputs_mr(batched_model_inputs, device, non_blocking=False):
    ...
    # print('batched model inputs', batched_model_inputs)
    targets = {}
